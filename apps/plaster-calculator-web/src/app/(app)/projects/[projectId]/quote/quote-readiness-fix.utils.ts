 ...
 * that editor already uses. Grouped in this module so the read side
 * (`findAreaInReadinessData`, seeding a control's current value from the
 * already-fetched `GetQuoteReadiness` response) and the write side
 * (`patchArea`, called immediately after a fresh `ProjectsService.getPage`)
 * share one name instead of each re-deriving how an area is located inside
 * the overlay.
 */
 ...
 */
export function findAreaInReadinessData(
 ...
 */
export function currentReferencePoints(
...
export function currentWallBoardType(
    readinessData: GetQuoteReadinessData | undefined,
    item: ReadinessAffectedItem,
): WallBoardType {
    const area = findAreaInReadinessData(
        readinessData,
        item.pageId,
        item.areaId,
    );
...
export function currentCeilingHeightMm(
    readinessData: GetQuoteReadinessData | undefined,
    item: ReadinessAffectedItem,
): number | null {
    const area = findAreaInReadinessData(
...
export function pageDefaultCeilingHeightMm(
    readinessData: GetQuoteReadinessData | undefined,
    item: ReadinessAffectedItem,
): number {
    const page = readinessData?.floorplanPages.find(
 ...
 * number to render). */
export function currentUnitPriceCents(
    readinessData: GetQuoteReadinessData | undefined,
    item: ReadinessAffectedItem,
): number {
    const config = readinessData?.quoteItemTemplateConfigs.find(
        (candidate) => candidate.itemTemplateId === item.quoteItemTemplateId,
    );
...
export function currentQuestionnaireAnswer(
    readinessData: GetQuoteReadinessData | undefined,
    item: ReadinessAffectedItem,
): string {
