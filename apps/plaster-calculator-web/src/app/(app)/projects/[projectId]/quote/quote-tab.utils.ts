 ...
 */
export function toEditableValues(quote: ProjectQuote): EditableQuoteFormValues {
    return {
...
export function toAppearance(
    ...
    return resolveQuoteAppearance(
        row ? QuoteAppearanceSchema.parse(toRawAppearance(row)) : undefined,
    );
}

/** Normalises `GetProjectQuote`'s optional appearance fields down to the nullable shape `QuoteAppearanceSchema` expects. */
function toRawAppearance(row: ProjectQuoteAppearanceRow): RawQuoteAppearance {
    return {
...
export function toDocumentProps({
    ...
    return {
        ...
        takeoffSummaryText: takeoffSummaryText(quote),
        lineItems: quote.items.map((item) => ({
            id: item.id,
            name: item.name,
            quantity: item.quantity,
 ...
 */
function takeoffSummaryText(quote: ProjectQuote): string | undefined {
...
function quantitySourceLabel(
    ...
    const source = humanizeMeasurement(quantitySource.measurementSource);
    return quantitySource.measurementPlasterType
        ? `${source} — ${humanizeMeasurement(quantitySource.measurementPlasterType)}`
        : source;
}

function formatTakeoffSummaryEntry(entry: TakeoffSummaryEntry): string {
...
function humanizeMeasurement(value: string): string {
    return value
        .toLowerCase()
        .split("_")
        .filter(Boolean)
