 ...
 * `plaster-calculator-ui`) sends the user to. Kept as static methods (not
 * floating functions) so the quote tab (building an `href`) and the
 * floorplan route (parsing `searchParams`) share one name instead of each
 * re-deriving the param contract. `tool` reuses `EditorInitialTool` from
 * the editor package itself, rather than a locally duplicated literal
 * union, so a value this class considers valid is guaranteed to be one the
 * editor actually understands.
 */
export class FloorplanDeepLinkUtils {
    /**
