...
export default function ThemeSettingsButton() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("plaster-theme") as ThemeMode | null;
        ...
        <div className="modal-backdrop">
          <section className="modal settings-modal">
            <header className="editor-toolbar">
              <div>
                <h2>Settings</h2>
                <p className="muted">Choose the colour theme for this browser.</p>
              </div>
              <button className="btn icon" onClick={() => setSettingsOpen(false)} title="Close settings">
                <X size={18} />
              </button>
            </header>
            <div className="field">
              ...
              <div className="segmented wide">
                {(["system", "light", "dark"] as ThemeMode[]).map((mode) => (
                  <button key={mode} className={themeMode === mode ? "active" : ""} onClick={() => setThemeMode(mode)}>
