...
import { centsToAudDisplayText } from "../../src/index.ts";

test("centsToAudDisplayText formats a simple sub-dollar amount", () => {
    assert.equal(centsToAudDisplayText(500), "$5.00");
});

test("centsToAudDisplayText inserts a thousands separator", () => {
    assert.equal(centsToAudDisplayText(123456), "$1,234.56");
});
...
test("centsToAudDisplayText formats zero", () => {
    assert.equal(centsToAudDisplayText(0), "$0.00");
});
