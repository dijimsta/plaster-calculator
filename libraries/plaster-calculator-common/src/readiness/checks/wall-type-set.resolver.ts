 ...
 * `AreaPolygon.wallBoardType` — i.e.
 * `BoardMaterialsHelper.wallBoardTypeSource()` reports `"explicit"`, not a
 * legacy-field mapping or a default. Outdoor areas are exempt, mirroring the
 * export validation in `plaster-calculator-ui`'s `validatePageForExport()`,
 * which never requires a wall board type for them.
 */
