import { computeQuantities } from "./quantity-takeoff-calculator.utils.ts";
...
/**
 * Combines every floorplan page's `computeQuantities()` result (see
 * `quantity-takeoff-calculator.utils.ts`) into one quantity per
 * `QuantitySource` for the whole project. A project can have several pages,
 * each with its own scale and overlay, but one quote covers the whole
 * project — `Quote.project` is `@unique` (`data/schemas/quotes.gql`) and
 * `QuoteItem` has no per-page field, so the schema only supports a single
 * set of summed line items, not one take-off per page. Summing (rather
 * than, say, keeping per-page totals alongside the sum) is the resolved
 * design for WORK-142.
 *
...
export function rollup(
    ...
    const contributingPages = pages.filter(pageContributes);
    const contributingPageIds = contributingPages.map((page) => page.pageId);
    ...
    return quantitySources.map((source) => ({
        ...
        quantity: perPageResults.reduce(
            (total, pageResults) => total + quantityFor(pageResults, source.id),
            0,
        ),
        contributingPageIds,
    }));
...
function quantityFor(
    pageResults: ReturnType<typeof computeQuantities>,
