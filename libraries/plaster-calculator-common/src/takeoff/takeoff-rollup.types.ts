...
/**
 * One floorplan page's inputs to `rollup()` (`takeoff-rollup.utils.ts`) —
 * the same three per-page values `computeQuantities()`
 * (`quantity-takeoff-calculator.utils.ts`) already takes (`overlay`,
 * `scaleMmPerPx`, `pageHeightMm`), plus `pageId` for provenance. Mirrors
 * `FloorplanPage` (`data/schemas/projects.gql`: `id`, `scaleMmPerPx`,
 * `ceilingHeightMm`) rather than reusing that type directly, for two
 * reasons: `overlay` here is the already-parsed `Overlay` that
 * `computeQuantities()` expects, not `FloorplanPage.overlay`'s raw JSON
 * string (parsing it is the caller's job, the same boundary
 * `parseOverlayAreas()` draws for the readiness gate); and `pageHeightMm`
 * is named for what it means to the calculator rather than mirroring the
 * schema's `ceilingHeightMm` column name.
 */
 ...
 * fed into `quantity` — i.e. every `PageTakeoffInput` `rollup()` did not
 * skip (see its doc comment for the skip conditions) — so a
