...
/**
 * Builds the single searchable text corpus that `match()` (see
 * `quote-item-keyword-matcher.utils.ts`) searches, out of a project's
 * `extractedTextJson` (PDF text, JSON-encoded per page) and its pages'
 * `ocrTextContent` (plain text per page). Kept separate from
 * `quote-item-keyword-matcher.utils.ts` because it is a distinct concern —
 * parsing/collecting project text — that a caller could also want
 * independently of keyword matching (e.g. to feed an AI flow, the way
 * `questionnaire-ai.ts`'s `resolveExtractedText()`/`buildOcrText()` already
 * do outside this package).
 */
 ...
 * than throwing, matching `parseOverlayAreas()`'s defensive handling of
 * `FloorplanPage.overlay` elsewhere in this package.
 */
export function buildSearchableCorpus(project: ProjectPlanTextSource): string {
    const extractedTextPages = parseExtractedText(project.extractedTextJson);
...
function parseExtractedText(
    ...
    try {
        ...
        return parsed
            .map((page: unknown) => (page as { text?: unknown } | null)?.text)
            .filter(
                (text): text is string =>
