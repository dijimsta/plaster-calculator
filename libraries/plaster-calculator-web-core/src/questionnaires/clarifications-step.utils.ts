 ...
 * (matching `generate-quote.utils.ts`'s "thin hook, pure utils" split) so
 * every branch is testable without a `DataConnect` instance.
 */
...
export function toClarificationRows(
    ...
    return questions.map((question) => {
        ...
        return {
            ...
            answer,
            answerSource,
            status: deriveRowStatus(answer, answerSource),
        };
    });
}
...
export function countNotAnsweredOnPlan(
...
export function nextPositionAfter(rows: readonly ClarificationRow[]): number {
 ...
 * `buildMutationVariables()`'s (`generate-quote.utils.ts`) "fail loudly
 * rather than silently drop rows" precedent for the same fixed-slot
 * mutation shape.
 */
export function buildBatchApplyTemplateVariables(
    projectId: string,
    ...
    questions.forEach((question, index) => {
        ...
        variables[`question${String(slot)}Position`] = startingPosition + index;
    });
    ...
    return {
        ...
        // branches, matching `assignSlot()`'s (`generate-quote.utils.ts`)
        // cast.
        variables:
