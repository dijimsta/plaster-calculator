...
export function evaluate(
    ...
    const input = buildReadinessCheckInput(data, defaultTemplateConfigs);
    return input ? READINESS_CHECKS.map((check) => check.resolve(input)) : [];
}
 ...
 */
export function isReady(results: readonly ReadinessResult[]): boolean {
 ...
 */
function buildReadinessCheckInput(
    data: GetQuoteReadinessData,
    defaultTemplateConfigs: readonly QueryTemplateConfig[],
    ...
    return {
        project: buildProjectDetail(data.project, data.floorplanPages),
        ...
        company: buildCompany(data.project.company),
    };
}
 ...
 * anything off `project` besides `pages` (via `readiness-check.utils.ts`) —
 * `COMPANY_CONTACT_DETAILS` (WORK-221/222) is the one exception, but it
 * reads `company` off the top-level `ReadinessCheckInput`
 * (`buildCompany()`, above), not off `project` itself. So the remaining
...
function buildProjectDetail(
    ...
    return {
        ...
        scope: null,
        pages: floorplanPages.map(buildFloorplanPage),
    };
}

/**
 * `FloorplanPage.overlay` is the only per-page field the resolvers read
 * besides `scaleMmPerPx`/`ceilingHeightMm` (via `readiness-check.utils.ts`),
 * so the rest of `FloorplanPageSchema`'s required fields are inert
 * placeholders here too — see `buildProjectDetail`.
 */
function buildFloorplanPage(page: QueryFloorplanPage): FloorplanPage {
    return {
        id: page.id,
        ...
        imageUrl: "",
        previewUrl: "",
        overlay: parseOverlayJson(page.overlayJson),
 ...
 * unchanged if valid, so downstream area-parsing (`activeAreasForPage()` in
 * `readiness-check.utils.ts`) gets exactly what it already expects: a JSON
 * string it can re-parse, or `null`. Null, unparsable, and schema-invalid
 * JSON all degrade to `null` rather than throwing — `parseOverlayAreas()`
 * already treats a `null` overlay as zero areas, which is what makes
 * `ROOMS_MEASURED` (and the other area-level checks) correctly report
 * "unmet" for a malformed page instead of crashing the whole gate.
 */
function parseOverlayJson(
